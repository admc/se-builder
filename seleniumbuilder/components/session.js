...
goog.string.compareVersions = function(a, b) {
  for(var c = 0, d = goog.string.trim("" + a).split("."), e = goog.string.trim("" + b).split("."), g = Math.max(d.length, e.length), f = 0;0 == c && f < g;f++) {
    var h = d[f] || "", k = e[f] || "", l = RegExp("(\\d*)(\\D*)", "g"), m = RegExp("(\\d*)(\\D*)", "g");
    do {
      var i = l.exec(h) || ["", "", ""], j = m.exec(k) || ["", "", ""];
      if(0 == i[0].length && 0 == j[0].length) {
        break
      }
      var c = 0 == i[1].length ? 0 : parseInt(i[1], 10), n = 0 == j[1].length ? 0 : parseInt(j[1], 10), c = goog.string.compareElements_(c, n) || goog.string.compareElements_(0 == i[2].length, 0 == j[2].length) || goog.string.compareElements_(i[2], j[2])
    }while(0 == c)
  }
  return c
};
...
bot.userAgent.FIREFOX_EXTENSION = function() {
  ...
  var c = a.classes, a = a.interfaces, d = c["@mozilla.org/xpcom/version-comparator;1"].getService(a.nsIVersionComparator), c = c["@mozilla.org/xre/app-info;1"].getService(a.nsIXULAppInfo), e = c.platformVersion, g = c.version;
...
var fxdriver = {moz:{}}, CC = Components.classes, CI = Components.interfaces, CR = Components.results, CU = Components.utils;
