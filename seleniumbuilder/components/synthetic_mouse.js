...
bot.events.TouchEventFactory_.prototype.create = function(a, b) {
  ...
  var f = goog.dom.getOwnerDocument(a), g = goog.dom.getWindow(f), h = e(b.changedTouches), j = b.touches == b.changedTouches ? h : e(b.touches), m = b.targetTouches == b.changedTouches ? h : e(b.targetTouches), i;
  bot.events.BROKEN_TOUCH_API_ ? (i = f.createEvent("MouseEvents"), i.initMouseEvent(this.type_, this.bubbles_, this.cancelable_, g, 1, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, b.metaKey, 0, b.relatedTarget), i.touches = j, i.targetTouches = m, i.changedTouches = h, i.scale = b.scale, i.rotation = b.rotation) : (i = f.createEvent("TouchEvent"), goog.userAgent.product.ANDROID ? i.initTouchEvent(j, m, h, this.type_, g, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, 
  b.metaKey) : i.initTouchEvent(this.type_, this.bubbles_, this.cancelable_, g, 1, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, b.metaKey, j, m, h, b.scale, b.rotation), i.relatedTarget = b.relatedTarget);
  return i
};
...
goog.dom.browserrange.IeRange.prototype.getEndpointNode_ = function(a, b) {
  ...
  for(var d = goog.dom.RangeEndpoint.START, e = goog.dom.RangeEndpoint.END, f = a == d, g = 0, h = c.childNodes.length;g < h;g++) {
    var j = f ? g : h - g - 1, m = c.childNodes[j], i;
    try {
      i = goog.dom.browserrange.createRangeFromNodeContents(m)
    }catch(n) {
      continue
    }
    var l = i.getBrowserRange();
    if(this.isCollapsed()) {
      if(goog.dom.browserrange.canContainRangeEndpoint(m)) {
        if(i.containsRange(this)) {
          return this.getEndpointNode_(a, m)
        }
      }else {
...
SyntheticMouse.prototype.move = function(a, b, c) {
  this.lastElement = a = a || this.lastElement;
  var b = b || 0, c = c || 0, d = goog.dom.getOwnerDocument(a);
  goog.dom.getWindow(d);
  bot.setWindow(goog.dom.getWindow(d));
  var e = new bot.Mouse, d = bot.action.scrollIntoView(a, new goog.math.Coordinate(b, c));
  if(!bot.dom.isElement(a, goog.dom.TagName.OPTION) && !d) {
    return SyntheticMouse.newResponse(bot.ErrorCode.MOVE_TARGET_OUT_OF_BOUNDS, "Element cannot be scrolled into view:" + a)
  }
  var f = d = 0;
  this.isButtonPressed && (d = goog.dom.getOwnerDocument(a), f = goog.dom.getDomHelper(d).getDocumentScroll(), d = 2 * (f.x - this.viewPortOffset.x), f = 2 * (f.y - this.viewPortOffset.y));
  b = new goog.math.Coordinate(b + d, c + f);
  e.move(a, b);
  return SyntheticMouse.newResponse(bot.ErrorCode.SUCCESS, "ok")
};
SyntheticMouse.prototype.click = function(a) {
  var a = a ? a : this.lastElement, b = this.isElementShown(a);
  if(b) {
    return b
  }
  if("option" == a.tagName.toLowerCase()) {
    ...
    b && "select" == b.tagName.toLowerCase() && !b.multiple && bot.action.click(b)
  }
  fxdriver.Logger.dumpn("About to do a bot.action.click on " + a);
  bot.action.click(a);
  return SyntheticMouse.newResponse(bot.ErrorCode.SUCCESS, "ok")
};
SyntheticMouse.prototype.contextClick = function(a) {
  ...
  fxdriver.Logger.dumpn("About to do a bot.action.rightClick on " + a);
  bot.action.rightClick(a);
  return SyntheticMouse.newResponse(bot.ErrorCode.SUCCESS, "ok")
};
SyntheticMouse.prototype.doubleClick = function(a) {
  ...
  fxdriver.Logger.dumpn("About to do a bot.action.doubleClick on " + a);
  bot.action.doubleClick(a);
  return SyntheticMouse.newResponse(bot.ErrorCode.SUCCESS, "ok")
};
SyntheticMouse.prototype.down = function(a) {
  var b = this.getElement_(a), c = goog.style.getClientPosition(b);
  this.isButtonPressed = !0;
  var d = goog.dom.getOwnerDocument(b);
  this.viewPortOffset = goog.dom.getDomHelper(d).getDocumentScroll();
  this.buttonDown = bot.Mouse.Button.LEFT;
  bot.events.fire(b, bot.events.EventType.MOUSEDOWN, {clientX:a.x + c.x, clientY:a.y + c.y, button:bot.Mouse.Button.LEFT});
  return SyntheticMouse.newResponse(bot.ErrorCode.SUCCESS, "ok")
};
SyntheticMouse.prototype.up = function(a) {
  var b = this.getElement_(a), c = goog.style.getClientPosition(b), a = {clientX:a.x + c.x, clientY:a.y + c.y, button:this.buttonDown};
  bot.events.fire(b, bot.events.EventType.MOUSEMOVE, a);
  bot.events.fire(b, bot.events.EventType.MOUSEUP, a);
